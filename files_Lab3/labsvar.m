...
% Plot CIELAB values for the light sources and objects under different illuminations
plot_Lab;

% Teori: The CIELAB values of all three light sources are (L, a, b*) = (100, 0, 0).**
% This happens because in the CIELAB color space, the white point of any light source is always mapped to the same position (100, 0, 0), regardless of its spectral distribution.
% This is why only one black circle is visible in the plot, even though three different light sources are present.

%% 5.7

% Plot CIELAB values for the objects under different illuminations
plot_Lab;

% Teori:
% Yes, in CIELAB space, the position of each object is much more independent of the illumination compared to CIEXYZ space.
% While objects still move slightly when the illumination changes, the relative distances between them remain more stable.
% This is because CIELAB is perceptually uniform, meaning that it better represents how the human eye perceives color differences.
% In contrast, in CIEXYZ space, object positions shifted significantly under different illuminants, making color comparisons difficult.


% 5.8 svarades direkt i pdf.